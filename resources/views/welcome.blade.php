...
<body class="antialiased font-sans text-gray-900 bg-gray-50">
    ...
    <nav class="absolute top-0 left-0 w-full z-50 bg-transparent py-6">
        <div class="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
            <div class="text-white font-black text-2xl tracking-tighter flex items-center">
                <svg class="w-8 h-8 mr-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                LECS.
            </div>
            <div class="space-x-4 flex items-center">
                <a href="{{ route('events.index') }}" class="text-white hover:text-gray-300 font-medium transition-colors hidden sm:block">Explore Events</a>
                @if (Route::has('login'))
                    @auth
                        <a href="{{ route('dashboard') }}" class="text-white hover:text-gray-300 font-medium transition-colors ml-6">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="text-white hover:text-gray-300 font-medium transition-colors ml-6">Log in</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="ml-4 px-5 py-2.5 rounded-full bg-white text-black font-bold hover:bg-gray-100 hover:shadow-lg transition-all transform hover:-translate-y-0.5">Register</a>
                        @endif
                    @endauth
                @endif
            </div>
    ...
    <div class="relative overflow-hidden bg-gray-900 pt-32 pb-24 lg:pt-48 lg:pb-32">
        <div class="absolute inset-0">
            <div class="absolute inset-0 bg-gradient-to-br from-black via-black to-black mix-blend-multiply opacity-80"></div>
            <!-- Decorative blobs -->
            <div class="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 rounded-full bg-gray-700 opacity-20 blur-3xl filter animate-pulse"></div>
            <div class="absolute bottom-0 left-0 -ml-20 -mb-20 w-80 h-80 rounded-full bg-gray-800 opacity-20 blur-3xl filter animate-pulse delay-700"></div>
        ...
        <div class="relative max-w-7xl mx-auto px-6 md:px-12 text-center">
            <h1 class="text-4xl md:text-6xl lg:text-7xl font-extrabold text-white tracking-tight mb-6 drop-shadow-xl">
                Discover Amazing <br/><span class="text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-gray-500">Local Events</span>
            </h1>
            <p class="mt-4 text-xl md:text-2xl text-gray-200 max-w-3xl mx-auto mb-10 font-light drop-shadow-md">
            ...
            <div class="mt-10 flex justify-center gap-3 flex-wrap">
                <span class="text-gray-300 text-sm font-medium mr-2 self-center">Popular:</span>
                @foreach($categories->take(4) as $category)
                    <a href="{{ route('events.index', ['category' => $category->id]) }}" class="px-4 py-1.5 rounded-full border border-gray-400/30 text-gray-200 text-sm hover:bg-gray-800/20 hover:border-gray-400 backdrop-blur-sm transition-colors">
                        {{ $category->name }}
                    </a>
                @endforeach
            </div>
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-12 py-20">
        <div class="flex justify-between items-end mb-12">
            ...
            <a href="{{ route('events.index') }}" class="hidden sm:inline-flex items-center text-gray-900 font-bold hover:text-black group">
                View all events 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($upcomingEvents as $event)
                <div class="bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 group">
                    ...
                    <div class="p-6 relative">
                        <div class="absolute -top-6 right-6 h-12 w-12 bg-white rounded-full flex items-center justify-center shadow-md border border-gray-50 text-gray-600">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                        </div>
                        <a href="{{ route('events.show', $event) }}" class="block group-hover:text-gray-900 transition-colors">
            ...
            @empty
                <div class="col-span-full py-12 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
                    ...
                    @auth
                        <a href="{{ route('events.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-900 text-white rounded-lg font-medium shadow hover:bg-black">Create Event</a>
                    @else
                        <a href="{{ route('register') }}" class="inline-flex items-center px-4 py-2 bg-gray-900 text-white rounded-lg font-medium shadow hover:bg-black">Join to Create</a>
                    @endauth
                </div>
            @endforelse
        </div>
        ...
        <div class="mt-8 sm:hidden text-center">
            <a href="{{ route('events.index') }}" class="inline-flex items-center text-gray-900 font-bold hover:text-black">
                View all events 
    ...
    <div class="bg-gray-900">
        <div class="max-w-7xl mx-auto px-6 md:px-12 py-16 md:py-24 flex flex-col md:flex-row items-center justify-between">
            <div class="text-center md:text-left mb-8 md:mb-0">
                ...
                <p class="mt-4 text-xl text-gray-300">Join thousands of organizers planning local events.</p>
            </div>
            <div class="flex space-x-4">
                @auth
                    <a href="{{ route('events.create') }}" class="px-8 py-4 bg-white text-gray-900 font-bold rounded-full hover:bg-gray-50 hover:shadow-lg transition-all transform hover:-translate-y-1">Create Event Now</a>
                @else
                    <a href="{{ route('register') }}" class="px-8 py-4 bg-white text-gray-900 font-bold rounded-full hover:bg-gray-50 hover:shadow-lg transition-all transform hover:-translate-y-1">Get Started Free</a>
                @endauth
            </div>
        </div>
    </div>
