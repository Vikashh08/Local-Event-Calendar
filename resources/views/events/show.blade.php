    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-2xl text-gray-800 leading-tight">
                <a href="{{ route('events.index') }}" class="text-indigo-600 hover:text-indigo-800 mr-2 flex items-center inline-flex">
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-3xl border border-gray-100 mb-8">
                <div class="h-64 sm:h-80 md:h-96 w-full bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 relative flex items-end">
                    ...
                    <div class="relative z-10 p-8 sm:p-12 w-full text-white">
                        ...
                        <p class="text-lg text-indigo-100 font-medium">Organized by {{ $event->user->name }}</p>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        ...
                        <div class="lg:col-span-2 space-y-8">
                            <div>
                                <h3 class="text-2xl font-bold text-gray-900 mb-4 flex items-center">
                                    <svg class="w-6 h-6 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    About This Event
                                </h3>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                                <ul class="space-y-6">
                                    <li class="flex">
                                        <div class="flex-shrink-0 w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center border border-gray-100 text-indigo-600">
                                    ...
                                    <li class="flex">
                                        <div class="flex-shrink-0 w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center border border-gray-100 text-purple-600">
                                    ...
                                    <li class="flex">
                                        <div class="flex-shrink-0 w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center border border-gray-100 text-pink-600">
                            ...
                            <!-- RSVP Card -->
                            <div class="bg-white rounded-2xl shadow-lg border border-indigo-100 overflow-hidden relative group">
                                ...
                                <div class="p-6 relative z-10">
                                    ...
                                    @auth
                                        ...
                                        <form action="{{ route('rsvps.store', $event) }}" method="POST" class="space-y-3 mt-4">
                                            @csrf
                                            <button type="submit" name="status" value="yes" class="w-full relative flex items-center justify-center px-4 py-3 {{ $userRsvp?->status === 'yes' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50 border' }} rounded-xl font-semibold transition-all">
                                    ...
                                    @else
                                        ...
                                        <a href="{{ route('login') }}" class="block w-full text-center px-4 py-3 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700 shadow-sm transition">Log in to RSVP</a>
                                    @endauth
                                </div>
                            </div>
                            ...
                            <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                <h4 class="font-bold text-gray-900 mb-4 flex justify-between items-center">
                                    <span>Attendees</span>
                                    <span class="bg-indigo-100 text-indigo-800 text-xs py-1 px-2.5 rounded-full">{{ $event->rsvps->where('status', 'yes')->count() }} Going</span>
                                </h4>
                                @if($event->rsvps->where('status', 'yes')->count() > 0)
                                    <div class="flex -space-x-3 overflow-hidden">
                                        @foreach($event->rsvps->where('status', 'yes')->take(5) as $rsvp)
                                            <div class="inline-block h-10 w-10 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 ring-2 ring-white flex items-center justify-center text-white font-bold text-sm" title="{{ $rsvp->user->name }}">
